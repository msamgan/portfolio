  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const current =
          meta.current_page ?? json.current_page ?? json.page ?? page;
        const perPage =
          meta.per_page ??
          json.per_page ??
          json.pageSize ??
          (Array.isArray(list) ? list.length : 10);
      ...
      <section className="relative min-h-[40vh] flex items-center justify-center pt-24 pb-16 px-6 overflow-hidden">
        ...
        <div className="relative text-center max-w-4xl mx-auto space-y-6 animate-fade-in-up">
          ...
          <p className="text-lg md:text-xl text-[var(--color-muted)] max-w-2xl mx-auto leading-relaxed">
            Insights, tutorials, and updates from my work and open-source
            journey.
          </p>
          ...
          {!loading && posts.length > 0 && pagination.totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mt-12 pt-8 border-t border-white/10">
              ...
              <div className="flex items-center gap-3">
                <div className="px-4 py-2 rounded-full bg-white/5 border border-white/10 text-sm text-[var(--color-muted)]">
                  Page{" "}
