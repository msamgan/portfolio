...
export default function Navbar() {
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-cyan-500 to-transparent transition-opacity duration-500 ${
      ...
      <Container className="flex h-20 items-center justify-between">
          ...
          {links.map((l) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-gradient-to-r from-cyan-400 to-violet-400 rounded-full animate-pulse" />
                )}
                {/* Hover underline */}
                {!isActive && (
                  <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-[2px] bg-gradient-to-r from-cyan-400 to-violet-400 group-hover:w-3/4 transition-all duration-300 rounded-full" />
                )}
              </a>
            )
          })}
        ...
        >
          ...
          {/* Ripple effect ring */}
          <span className="absolute inset-0 rounded-xl ring-2 ring-cyan-400/0 group-hover:ring-cyan-400/30 transition-all duration-300" />
        </button>
      </Container>
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-500 ease-in-out ${
        ...
        <div className="border-t border-white/10 bg-gradient-to-b from-[var(--color-bg)]/95 to-[var(--color-surface)]/95 backdrop-blur-xl shadow-2xl">
              ...
              {links.map((l, index) => {
                ...
                return (
                  ...
                  >
                    <span>{l.label}</span>
                    {isActive ? (
                      <span className="w-2 h-2 bg-gradient-to-r from-cyan-400 to-violet-400 rounded-full animate-pulse" />
                    ) : (
